################################################################################
# This is the neural network that takes a sparse view sinogram as input.  Its
# first layer is a fully connected layer.  We used this network as a control
# group for comparison with the CNN contained in MCB_code.py and FMCB_code.py.
################################################################################
...
class Net(nn.Module):
    def __init__(self):
        ...
        stride_size = 1
        leak = 0.01
        # 95 is the number of channels in the input (for a 64x64 image, 64*sqrt(2)=95)
        # 16 is the number of views
        # will want to reshape the output into a stack of 64x64 images later,
        #   so that is the output dimension
        self.fc_in = nn.Linear(95*16, 64*64*width, bias=True)
        dev = math.sqrt(2/width)/ker_size
        nn.init.normal_(self.fc_in.weight, mean=0, std=dev)
    ...
    def forward(self, x):
        # we need to reshape the sinogram matrix into a vector
        x=x.view(1,-1)
        # input layer
        x = f.leaky_relu(self.fc_in(x))
        # we need to reshape the output vector of the FCL back into a matrix for
        #   the conv2d layers
        x=x.view(1,-1,64,64)
        # hidden layers
    ...
    def verbose_forward(self, x):
        ...
        x = f.leaky_relu(self.fc_in(x))
        num_chans = self.fc_in.weight.size()[0]
...
def train(model, device, datas, labels, data_size, batch_size, sin_rows, sin_cols, out_pixels, optimizer, epoch):
    ...
    for i in range(0, total_batches):
        ...
        data = torch.zeros([batch_size, 1, sin_rows, sin_cols])
...
def test(model, device, datas, labels, data_size, sin_rows, sin_cols, out_pixels):
    ...
    with torch.no_grad():
        for i in range(0, data_size):
            ...
            data = torch.zeros([1, 1, sin_rows, sin_cols])
            data[0] = datas[i]
...
def main():
    ...
    out_pixels = 64
    padding_size = 2
    # in_pixels = out_pixels + padding_size * 2  COMMENTED out bc we are using a sinogram
    sin_rows=95 ### sinogram input size
    sin_cols=16 ### sinogram input size
    datas = torch.zeros([data_size, 1, sin_rows, sin_cols]) ###### changed 16 to 1
    labels = torch.zeros([data_size, 1, out_pixels, out_pixels])
    ver_datas = torch.zeros([verify_size, 1, sin_rows, sin_cols]) ####### changed 16 to 1
    ...
    for i in range(0, data_size):
        ...
        # load data
        datas[i, 0, :, :] = torch.from_numpy( ####################### changed j to 0
        #numpy.array(PIL.Image.open('Dataset/in/mcb' + str(i + 1) + 'channel' + str(j + 1) + '.png'),dtype='float')) / 255
        numpy.array(PIL.Image.open('train/sinogram/sinogram' + str(i + 1) + '.png'),dtype='float')) / 255
    ...
    for i in range(0, verify_size):
        ...
        # load data
        ver_datas[i, 0, :, :] = torch.from_numpy( #################### changed j to 0
                #numpy.array(PIL.Image.open('verDataset/in/mcb' + str(i + 1) + 'channel' + str(j + 1) + '.png'),dtype='float')) / 255
            numpy.array(PIL.Image.open('test/sinogram/sinogram' + str(i + 1) + '.png'),dtype='float')) / 255
    ...
    for epoch in range(current_epoch, args.epochs + current_epoch):
        ...
        train_error[epoch - 1] = train(model, device, datas, labels, data_size, args.batch_size, sin_rows, sin_cols, out_pixels, optimizer, epoch)
        test_error[epoch - 1] = test(model, device, ver_datas, ver_labels, verify_size, sin_rows, sin_cols, out_pixels)
    ...
    model = model.to("cpu")
    original_image = torch.zeros([1, 1, sin_rows, sin_cols])
    ...
    feed_data = torch.zeros([1, 1, sin_rows, sin_cols])
    feed_data[0] = datas[0]

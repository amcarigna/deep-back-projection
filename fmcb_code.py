################################################################################
# This is the neural network that takes a stack of filtered single view back
# projections as input.  All layers are convolutional.  We tested this network 
# against the networks contained in MCB_code.py and FCL_code.py.
################################################################################
...
def main():
    ...
    ### EPOCHS #################################################################
    parser.add_argument('--epochs', type=int, default=153, metavar='N',
                        help='number of epochs to train (default: 153)')
    ############################################################################
        ...
        for j in range(0, 16):
            ...
            datas[i, j, :, :] = torch.from_numpy(
                #numpy.array(PIL.Image.open('Dataset/in/mcb' + str(i + 1) + 'channel' + str(j + 1) + '.png'),
                            #dtype='float')) / 255
                numpy.array(PIL.Image.open('train/fmcb/fmcb' + str(i + 1) + 'channel' + str(j + 1) + '.png'),
                            dtype='float')) / 255
        ...
        for j in range(0, 16):
            ...
            ver_datas[i, j, :, :] = torch.from_numpy(
                #numpy.array(PIL.Image.open('verDataset/in/mcb' + str(i + 1) + 'channel' + str(j + 1) + '.png'),
                            #dtype='float')) / 255
                numpy.array(PIL.Image.open('test/fmcb/fmcb' + str(i + 1) + 'channel' + str(j + 1) + '.png'),
                            dtype='float')) / 255
    ...
    for epoch in range(current_epoch, args.epochs + current_epoch):
        if epoch == 37:
            optimizer = optim.Adam(model.parameters(), lr=args.lr / 10,eps=0.01)  #Cut LR down
        if epoch == 75:
            optimizer = optim.Adam(model.parameters(), lr=args.lr / 10,eps=0.01)  #Cut LR down
        if epoch == 112:
            optimizer = optim.Adam(model.parameters(), lr=args.lr / 10,eps=0.01)  #Cut LR down
